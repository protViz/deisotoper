...
One chemical element can have different atomic masses because each chemical element can have a different number of neutrons.

The elementary mass of one neutron is approximately 1 Dalton.
The chemical elements hydrogen,  carbon, oxygen,  nitrogen, and sulfate composing chemical structures known as amino acids. Multiple amino acids sequences construct peptide and protein sequences. 
...
The plot below shows an isotopic cluster of a fragment ion having a monoisotopic peak of 362.02 Dalton measured on a Thermo Fisher Q Exactive HF Hybrid Quadrupole-Orbitrap mass spectrometer.
...
\subsection{Peak peaking}

Fit first isotope using \code{lm}.
...
\section{Find the isotop cluster}
