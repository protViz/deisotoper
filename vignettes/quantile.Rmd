...
loop <- function(start, stop) {
  for(i in start:stop) {
    ...
    IMS <- jCreateIMS(MS)
    MSd <- jDeisotopeMS(MS)
    MSd2 <- jDeisotopeMS(MS, configuration = jCreateConfiguration(DECHARGE = TRUE))
    ...
    if(IMS$getIsotopicMassSpectrum()$size() > 1) {
      for(j in 0:size2) {
        ...
        max <- min + 5 + 2 # peaks[[length(peaks)]]$getIsotopicCluster()$get(as.integer(peaks[[length(peaks)]]$getIsotopicCluster()$size() - 1))$getMz()
